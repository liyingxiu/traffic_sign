...
def run_training():
    ...
    # 进行前向训练，获得回归值
        ...
        train_prediction, result = net_model.inference(train_batch, BATCH_SIZE, N_CLASSES)
    # 计算获得损失值loss
    ...
    # 启动入队的线程，一般情况下，系统有多少个核，就会启动多少个入队线程（入队具体使用多少个线程在tf.train.batch中定义）
        ...
        try:
            ...
            # 保存一次训练得到的模型
                if (step + 1) == 30 or step == 20000 or step == 30000:
                    #checkpoint_path = os.path.join(logs_train_dir, 'net_model.ckpt')
                    #saver.save(sess, checkpoint_path, global_step=step)
                    with tf.gfile.FastGFile(logs_train_dir + 'model.pb', mode='wb') as f:
