...
def backward():
    ...
    x = tf.placeholder(tf.float32, [
        ...
        208,
        3])
    y_ = tf.placeholder(tf.float32, [None, mnist_forward_test.OUTPUT_NODE])
    y = mnist_forward_test.forward(x, True, REGULARIZER)  # 调用前向传播网络得到维度为10的tensor
    ...
    with tf.Session() as sess:  # 创建一个会话，并通过 python 中的上下文管理器来管理这个会话
        ...
        for i in range(STEPS):
            xs, ys = read_data.get_batch(image_H=208, image_W=208,batch_size=BATCH_SIZE, capacity=7500)  # 读取一个 batch 的数据
            coord = tf.train.Coordinator()
            thread = tf.train.start_queue_runners(sess, coord)
            # xs = xs.eval(session=sess)
            # ys = ys.eval(session=sess)
            xs, ys = sess.run([xs, ys])
            # reshaped_xs = np.reshape(xs, (  # 将输入数据 xs 转换成与网络输入相同形状的矩阵
            # BATCH_SIZE,
            # mnist_forward_test.IMAGE_SIZE,
            # mnist_forward_test.IMAGE_SIZE,
            # mnist_forward_test.NUM_CHANNELS))
        # 喂入训练图像和标签，开始训练
