...
y_ = tf.placeholder(tf.float32, [None, net_forward.OUTPUT_NODE])
...
ce = (logits=y, labels=tf.argmax(y_, 1))
...
#learning_rate = tf.train.exponential_decay(LEARNING_RATE_BASE, global_step, mnist.train.num_examples / BATCH_SIZE, LEARNING_RATE_DECAY, staircase=True)  # 当 staircase=True 时，（global_step/decay_steps）则被转化为整数，以此来选择不同的衰减方式
...
for i in range(STEPS):
    xs, ys = read_data.get_files()  # 读取一个 batch 的数据,不确定是否会出错
...
def main():
    traffic_sign = input_data.read_data_sets("./data/", one_hot=True)  # 读入 mnist 数据
    backward(traffic_sign)
